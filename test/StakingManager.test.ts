...
describe("StakingManager", () => {
  ...
  beforeEach(async function () {
    ...
    stakingConfigurator = await stakingConfiguratorDeployer({
      ...
      endTime: stakingStartTime + DAY_IN_SECONDS * 365 * 2,
  ...
  describe("Deposit", function () {
    ...
    beforeEach(async function () {
      ...
      await time.increaseTo(stakingStartTime);
    });
    ...
    it("allows a user to deposit tokens into a pool", async function () {
      ...
      // Simulate passing time to the middle of pool duration
      ...
      const rewardsAmount = await stakingManager.getRewardsForStake(user1, 0);
      expect(rewardsAmount).to.be.gt(0n);
  ...
  describe("Deposit out of period", function () {
    const poolId = 0;
    const depositAmount = ethers.parseEther("10000");
    ...
    it("reverts when staking period not yet started", async function () {
      await expect(stakingManager.connect(user1).deposit(poolId, depositAmount)).to.be.revertedWith(
        "Staking not started",
      );
    });

    it("reverts when staking period has ended", async function () {
      // Simulate passing time when the staking period has ended
      const endTime = await stakingConfigurator.getEndTime();
      await time.increaseTo(endTime + 10000n);

      await expect(stakingManager.connect(user1).deposit(poolId, depositAmount)).to.be.revertedWith("Staking ended");
    });
  });
  ...
  describe("Withdraw", function () {
    ...
    it("allows a user to withdraw tokens and claim rewards after the staking period", async function () {
      // Simulate passing time when the pool duration has ended
      const latestTime = await time.latest();
      const poolDuration = await stakingConfigurator.getPoolDuration(poolId);
      await time.increaseTo(latestTime + Number(poolDuration));
