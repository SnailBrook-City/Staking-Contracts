...
contract StakingManager is IStakingManager, ReentrancyGuard {
    ...
    constructor(IERC20 _token, IStakingConfigurator _configurator, IStakingRewardsManager _rewardsManager) {
        ...
        require(address(_configurator) != address(0), "StakingConfigurator address cannot be zero");
        require(address(_rewardsManager) != address(0), "StakingRewardsManager address cannot be zero");
    ...
    function deposit(uint8 poolId, uint256 amount) external nonReentrant {
        ...
        require(block.timestamp > configurator.getStartTime(), "Staking not started");
        require(block.timestamp < configurator.getEndTime(), "Staking ended");
